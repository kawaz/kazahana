# 64bit ID難読化アルゴリズム調査

調査日: 2026-02-02
関連チャット:
- 「PCG疑似乱数生成器の説明」(2026-02-02)
- 「64ビットIDの難読化アルゴリズム」(2026-02-01)
- 「UUIDとシリアルのトレードオフ解決策」(2026-02-01)

## 調査目的

Snowflake形式の64bit内部IDを外部公開用に難読化する全射（bijective）アルゴリズムの選定。

### 要件

1. **全射（可逆）**: 内部ID ↔ 外部ID の相互変換が可能
2. **拡散性**: 連番入力でも出力がランダムに見える
3. **高速**: ID生成のボトルネックにならない
4. **実装容易**: 依存なしで各言語に移植可能
5. **64bit維持**: 入出力ともに64bit

---

## 調査対象アルゴリズム

### 非暗号系（鍵なし）

#### 1. 算術系（単体では不十分）

| 名前 | 構造 | 拡散性 | 備考 |
|------|------|--------|------|
| XOR定数 | `x ^ K` | × | 最速、拡散性なし |
| 奇数乗算 | `x * K` (K奇数) | △ | 線形性あり |
| 加算 | `x + K` | × | 拡散性なし |
| ビット回転 | `rotl(x, n)` | × | 構造保存 |

これらは組み合わせて使う。単体では難読化に不十分。

#### 2. PRNGミキシング関数

| 名前 | 由来 | 発表年 | 拡散性 | 逆関数 |
|------|------|--------|--------|--------|
| **SplitMix64** | Java SplittableRandom | 2014 | ◎ | あり |
| **moremur** | Pelle Evensen | 2020 | ◎◎ | あり |
| MurmurHash3 finalizer | MurmurHash3 | 2012 | ◎ | あり |
| xxHash finalizer | xxHash | 2014 | ◎ | あり |
| wyhash finalizer | wyhash | 2019 | ◎◎ | あり |
| Stafford Mix13 | 改良版SplitMix | 2011 | ◎ | あり |
| degski | SplitMix改良 | 2018 | ◎ | あり |
| rrxmrrxmsx_0 | Pelle Evensen | 2018 | ◎ | あり |
| nasam | Pelle Evensen | 2020 | ◎◎ | あり |

#### 3. PRNG本体（状態遷移関数）

| 名前 | 構造 | 特徴 |
|------|------|------|
| xorshift64 | XOR + シフト | 古典、高速 |
| LCG (線形合同法) | 乗算 + 加算 | 全射だが予測しやすい |
| PCG | LCG + 出力変換 | 高品質 |
| SFC64 | カウンタ + 回転 | PractRand合格 |
| romu | 回転 + 乗算 | 2020年、高速 |
| wyrand | wyhash由来 | 超高速（128bit乗算必要） |

### 暗号系（鍵あり）

#### 1. 古典的64bitブロック暗号

| 名前 | 構造 | ラウンド | 特徴 |
|------|------|---------|------|
| **XTEA** | Feistel | 32 (推奨64) | シンプル、ARX、実績あり |
| TEA | Feistel | 32 | XTEAの前身、脆弱性あり |
| Blowfish | Feistel | 16 | Sボックス、鍵セットアップ重い |
| CAST-128 | Feistel | 12/16 | Sボックス4個 |
| GOST 28147-89 | Feistel | 32 | ロシア標準 |
| DES | Feistel | 16 | 56bit鍵、非推奨 |
| 3DES | Feistel×3 | 48 | DES 3回、遅い |
| Skipjack | 非Feistel | 32 | NSA、80bit鍵 |
| RC5-64 | ARX | 12-20 | 可変ラウンド、特許切れ |

#### 2. 軽量暗号（IoT向け）

| 名前 | 構造 | ラウンド | 特徴 |
|------|------|---------|------|
| SIMON-64 | Feistel | 42-44 | NSA、AND/XOR/回転 |
| SPECK-64 | ARX | 26-27 | NSA、高速 |
| PRESENT | SPN | 31 | 超軽量、4bit Sボックス |
| GIFT-64 | SPN | 28 | PRESENTの改良 |
| TWINE | Feistel | 36 | NEC |
| HIGHT | GFN | 32 | 韓国標準、ARX |
| LEA | ARX | 24 | 韓国標準、高速 |
| CHAM | ARX | 80 | 軽量、シンプル |
| SPARX-64 | ARX | 24 | 2016年、解析しやすい設計 |
| RECTANGLE | SPN | 25 | ビットスライス向け |

---

## 詳細検討

### SplitMix64

Java 8 の `SplittableRandom` で採用されたミキシング関数。

```typescript
const MASK64 = 0xFFFF_FFFF_FFFF_FFFFn;
const MUL1 = 0xBF58476D1CE4E5B9n;
const MUL2 = 0x94D049BB133111EBn;
const MUL1_INV = 0x64046F0D4046F0D5n;
const MUL2_INV = 0x319642B2D24D8EC3n;

function splitmix64_encode(x: bigint): bigint {
  x = (x ^ (x >> 30n)) * MUL1 & MASK64;
  x = (x ^ (x >> 27n)) * MUL2 & MASK64;
  x = x ^ (x >> 31n);
  return x;
}

function splitmix64_decode(x: bigint): bigint {
  x = x ^ (x >> 31n) ^ (x >> 62n);
  x = x * MUL2_INV & MASK64;
  x = x ^ (x >> 27n) ^ (x >> 54n);
  x = x * MUL1_INV & MASK64;
  x = x ^ (x >> 30n) ^ (x >> 60n);
  return x;
}
```

**特徴**:
- 構造: XORシフト(30) → 乗算 → XORシフト(27) → 乗算 → XORシフト(31)
- 知名度が高い（Java標準）
- 0→0 となる性質あり（Snowflake IDでは問題なし）

**参考**:
- Guy L. Steele Jr., Doug Lea, Christine H. Flood. "Fast Splittable Pseudorandom Number Generators" (2014)
- http://dx.doi.org/10.1145/2714064.2660195

### moremur

Pelle Evensen による SplitMix64 の改良版。

```typescript
const MASK64 = 0xFFFF_FFFF_FFFF_FFFFn;
const MUL1 = 0x3C79AC492BA7B653n;
const MUL2 = 0x1C69B3F74AC4AE35n;
const MUL1_INV = 0xCBF29CE484222325n;
const MUL2_INV = 0xD0E31B248A51E425n;

function moremur_encode(x: bigint): bigint {
  x = (x ^ (x >> 27n)) * MUL1 & MASK64;
  x = (x ^ (x >> 33n)) * MUL2 & MASK64;
  x = x ^ (x >> 27n);
  return x;
}

function moremur_decode(x: bigint): bigint {
  x = x ^ (x >> 27n) ^ (x >> 54n);
  x = x * MUL2_INV & MASK64;
  x = x ^ (x >> 33n);
  x = x * MUL1_INV & MASK64;
  x = x ^ (x >> 27n) ^ (x >> 54n);
  return x;
}
```

**特徴**:
- 構造: XORシフト(27) → 乗算 → XORシフト(33) → 乗算 → XORシフト(27)
- SplitMix64 と同一構造、定数とシフト量が異なる
- より網羅的な探索により拡散性が改善

**参考**:
- https://mostlymangling.blogspot.com/2019/12/stronger-better-morer-moremur-better.html

### wyhash finalizer

128bit乗算を使用する高速ミキシング関数。

```typescript
function wymix(a: bigint, b: bigint): bigint {
  const full = a * b;
  const lo = full & MASK64;
  const hi = (full >> 64n) & MASK64;
  return lo ^ hi;
}

function wyhash_mix(x: bigint): bigint {
  return wymix(x ^ 0x2D358DCCAA6C78A5n, x ^ 0x8BB84B93962EACC9n);
}
```

**特徴**:
- 128bit乗算が必要
- ネイティブ環境（Rust/Go/C++）では最速
- JavaScript/WASM では遅い（BigIntエミュレーション）

### XTEA

Needham & Wheeler による64bitブロック暗号。

```typescript
function xtea_encrypt(v: [number, number], key: number[]): [number, number] {
  let [v0, v1] = v;
  let sum = 0;
  const delta = 0x9E3779B9;

  for (let i = 0; i < 32; i++) {
    v0 += (((v1 << 4) ^ (v1 >>> 5)) + v1) ^ (sum + key[sum & 3]);
    v0 = v0 >>> 0;
    sum = (sum + delta) >>> 0;
    v1 += (((v0 << 4) ^ (v0 >>> 5)) + v0) ^ (sum + key[(sum >>> 11) & 3]);
    v1 = v1 >>> 0;
  }

  return [v0, v1];
}

function xtea_decrypt(v: [number, number], key: number[]): [number, number] {
  let [v0, v1] = v;
  const delta = 0x9E3779B9;
  let sum = (delta * 32) >>> 0;

  for (let i = 0; i < 32; i++) {
    v1 -= (((v0 << 4) ^ (v0 >>> 5)) + v0) ^ (sum + key[(sum >>> 11) & 3]);
    v1 = v1 >>> 0;
    sum = (sum - delta) >>> 0;
    v0 -= (((v1 << 4) ^ (v1 >>> 5)) + v1) ^ (sum + key[sum & 3]);
    v0 = v0 >>> 0;
  }

  return [v0, v1];
}
```

**特徴**:
- ARX構造（Add, Rotate, XOR）
- Sボックス不要、実装シンプル
- 128bit鍵、64bitブロック
- 32ラウンド（標準）、64ラウンド（推奨）

**参考**:
- David J. Wheeler, Roger M. Needham. "Tea extensions" (1997)
- http://www.cix.co.uk/~klockstone/xtea.pdf

---

## ベンチマーク比較

### 速度（JavaScript / Node.js）

| 関数 | ops/sec (概算) | 相対速度 |
|------|---------------|---------|
| SplitMix64 | 〜2,500,000 | 1.0x |
| moremur | 〜2,400,000 | 0.95x |
| wyhash_mix | 〜1,500,000 | 0.6x |
| XTEA 32R | 〜200,000 | 0.08x |

**注**: JavaScriptでは128bit乗算が遅いため、wyhashの優位性がない。

### 拡散性（Avalanche特性）

1bit入力変化に対する出力変化bit数（理想: 32bit = 50%）

| 関数 | 平均反転bit数 | 理想との差 |
|------|--------------|-----------|
| SplitMix64 | 31.8 | 0.6% |
| moremur | 32.0 | 0.0% |
| wyhash_mix | 32.0 | 0.0% |

---

## 128bit乗算の環境別対応状況

### ネイティブ対応

| 環境 | 対応 | 方法 |
|------|------|------|
| x86_64 | ◎ | `MUL` 命令 |
| ARM64 | ◎ | `UMULH` 命令 |
| C/C++ | ◎ | `__uint128_t` (GCC/Clang) |
| Rust | ◎ | `u128` ネイティブ |
| Go | ◎ | `math/bits.Mul64` |

### エミュレーション必要

| 環境 | 対応 | 備考 |
|------|------|------|
| JavaScript | △ | BigIntで可能だが遅い |
| Java | △ | `Math.multiplyHigh` (Java 9+) |
| WebAssembly | △ | 64bit×2で分割計算 |
| 32bit環境 | × | 重いエミュレーション |

**結論**: 広範なプラットフォーム対応を考慮すると、128bit乗算に依存しないアルゴリズムが望ましい。

---

## パッケージ存在状況

### SplitMix64

| 言語 | パッケージ | 状況 |
|------|-----------|------|
| Rust | `rand_xoshiro` | ✅ Rand公式 |
| Go | `github.com/db47h/rand64/splitmix64` | ✅ |
| JavaScript | `@entropy-source/pseudo-random` | △ 古い |
| Java | 標準 `SplittableRandom` | ✅ |
| Python | なし | △ 自前実装 |

### moremur

| 言語 | パッケージ | 状況 |
|------|-----------|------|
| 全言語 | なし | ❌ 専用パッケージなし |

### XTEA

| 言語 | パッケージ | 状況 |
|------|-----------|------|
| Go | `golang.org/x/crypto/xtea` | ✅ 公式 |
| Rust | `xtea` | ✅ |
| JavaScript | `xtea` | ✅ |
| Python | `xtea` | ✅ |
| Java | なし | △ 自前実装 |

---

## NSA設計暗号への懸念

### 背景

2013年スノーデン事件で、NSAが暗号標準にバックドアを仕込んでいたことが発覚（Dual_EC_DRBG）。

### SIMON/SPECK への疑念

- 設計根拠が十分に公開されていない
- 学術解析では脆弱性は見つかっていない（70本以上の論文で解析済み）

### SPECK の ISO 標準化経緯

| 時期 | 出来事 |
|------|--------|
| 2013年6月 | NSAがSimon/Speckを公開 |
| 2015年 | ISO/IEC 29192-2（軽量暗号標準）への追加を提案 |
| 2018年4月 | **ISO/IEC JTC 1/SC 27で拒否** — ドイツ、イスラエル、日本の代表が反対。NSAの「敵対的」な態度が批判された |
| 2018年8月 | Linux kernelから削除（4.20） |
| 2018年10月 | **別のワーキンググループ（SC 31）でRFID標準として承認** — ISO/IEC 29167-22:2018 |

**重要な点**: 「暗号標準」（SC 27）としては拒否されたが、「RFID air interface標準」（SC 31）の一部として別ルートで採用された。同じISOでも異なる委員会での決定であり、「ISO標準になった」という表現は正確ではない。

### 結論

技術的には問題ない（full-round攻撃は発見されていない）が、心理的抵抗がある層も存在する。ライブラリとしては両方のオプションを提供し、ユーザーに選択させるのが現実的。

---

## Ascon の調査（NIST軽量暗号標準 2023）

### 概要

Asconは2023年にNISTが軽量暗号標準として採用したアルゴリズム。Graz工科大学のチームが設計。

### 構造

- **320bit permutation**（5×64bit）ベースの sponge/duplex 構造
- 主用途: AEAD（認証付き暗号）、ハッシュ
- **64bitブロック暗号ではない**

### 64bit全単射への適用可能性

| 方式 | 実現可能性 | 問題点 |
|------|-----------|--------|
| Ascon permutationをFeistelのラウンド関数に | 理論上可能 | 過剰設計、遅い |
| Ascon-PRFの出力を64bitに切り詰め | 可能 | **全単射ではなくなる** |
| 直接使用 | 不可能 | 320bit状態、64bitブロック暗号ではない |

### 性能比較

| 項目 | Ascon | Speck64/128 |
|------|-------|-------------|
| 設計 | 320bit sponge/duplex | 64bitブロック暗号 |
| 主用途 | AEAD、ハッシュ | 単独暗号化 |
| 64bit単発暗号 | ❌ 直接不可 | ✅ 可能 |
| 性能（長メッセージ） | ~8-11 cycles/byte | ~2.5 cycles/byte |
| NIST標準 | 2023年軽量暗号標準 | なし |

### 結論

**Asconは64bit ID obfuscationには不適**。AEAD/ハッシュ用途に最適化された設計であり、64bit全単射変換という要件を満たせない。

---

## 構造の比較

### SplitMix64 vs moremur

```
SplitMix64: (xor-shift 30) → mul → (xor-shift 27) → mul → (xor-shift 31)
moremur:    (xor-shift 27) → mul → (xor-shift 33) → mul → (xor-shift 27)
```

本質的には同じ構造。定数とシフト量の選び方が異なる。moremur は2020年の研究でより網羅的に探索して最適な組み合わせを発見。

### 抽象化

```typescript
function mix64(x: bigint, shifts: number[], muls: bigint[]): bigint {
  x = (x ^ (x >> BigInt(shifts[0]))) * muls[0] & MASK64;
  x = (x ^ (x >> BigInt(shifts[1]))) * muls[1] & MASK64;
  x = x ^ (x >> BigInt(shifts[2]));
  return x;
}
```

---

## 選定結果

### デフォルトモード（鍵なし）

**採用: moremur**

理由:
- SplitMix64 と同等の速度
- より良い拡散性（2020年の研究成果）
- 実装が簡単（20行程度）
- 特許・ライセンス問題なし
- 128bit乗算不要

### 暗号化モード（鍵あり）

**採用: XTEA（デフォルト）+ Speck64（オプション）**

両方を実装し、ユーザーが選択可能とする。

#### XTEA（64ラウンド）— デフォルト

| 項目 | 内容 |
|------|------|
| 設計者 | Wheeler & Needham（ケンブリッジ大学） |
| 公開年 | 1997年 |
| 構造 | 64bit Feistel + ARX |
| 推奨ラウンド | 64 |
| 速度 | ~67 cycles/byte（~15M ops/sec） |
| 信頼性 | ✅ 学術由来、27年の実績 |

採用理由:
- 学術機関由来で政治的に中立
- full-round攻撃は発見されていない
- 実装が最小（10行程度）
- 「なぜこの暗号？」への説明が容易

#### Speck64/128 — 高速オプション

| 項目 | 内容 |
|------|------|
| 設計者 | NSA |
| 公開年 | 2013年 |
| 構造 | 64bit ARX |
| ラウンド | 27 |
| 速度 | ~2.5 cycles/byte（~150M ops/sec） |
| 信頼性 | ⚠️ NSA由来だが、70本以上の論文で解析済み |

採用理由:
- XTEAの約10倍高速
- 大量処理が必要な場合に有用
- 技術的には安全（full-round攻撃なし）

NSA懸念を重視するユーザーはXTEAを使用すればよい。

#### 性能比較

| 処理量 | XTEA (64R) | Speck64/128 |
|--------|------------|-------------|
| 1秒あたり処理数 | ~15M ops/sec | ~150M ops/sec |
| 100万ID処理時間 | ~67ms | ~7ms |

実用上、100万ID/秒でも67ms。大量バッチ処理でなければXTEAで十分。

#### 推奨プリセット

```typescript
type EncryptionAlgorithm = 'xtea' | 'speck64';

const defaults = {
  // 信頼性重視（デフォルト）
  standard: 'xtea',

  // 速度重視（NSA懸念より速度を優先する場合）
  fast: 'speck64',
} as const;
```

---

---

## PCG（Permuted Congruential Generator）の調査

### 概要

PCGは2014年にM.E. O'Neillによって発表された優れた疑似乱数生成器ファミリー。
参考: https://www.pcg-random.org

### PCGの特徴

| 項目 | 評価 |
|------|------|
| 統計的品質 | 優秀（統計テストをよく通過） |
| 予測困難性 | 高い（多くのRNGより安全） |
| 速度 | 非常に高速 |
| メモリ使用量 | 非常にコンパクト |
| コードサイズ | 非常に小さい |
| 複数ストリーム | 対応（例: 2^63個） |

### PCGの仕組み

乱数生成器は2つの部分に分けられる：

1. **状態遷移関数**: 内部状態を次の状態に更新する
2. **出力関数**: 内部状態から実際の乱数を生成する

**従来のRNGの問題点**：
- 多くは出力関数が単純すぎる（状態をそのまま返すなど）→ 予測されやすい
- 暗号学的RNG（Fortunaなど）は出力関数が重すぎて遅い

**PCGのアプローチ**：
- **状態遷移**: LCG（線形合同法）を使用 → シンプルで高速
- **出力関数**: **Permutation（置換）関数**を適用 → 状態から予測困難な出力を生成

### PCGと単一値ミキシングの関係

PCGそのものは乱数列の生成向けであり、単一64bit値の可逆ミキシングには向いていない。
理由：PCGの代表的な出力関数はビット数を削減する（例: 128bit状態 → 64bit出力）ため不可逆。

ただし、PCGの設計思想（LCG + 出力変換）は、SplitMix64などのミキシング関数の設計に影響を与えている。

---

## 品質テスト結果（実測）

Cによる実測ベンチマーク結果。サンプル数: 100,000

### 1. Avalanche Test（平均バイアス）

1bit入力変化に対する出力変化の偏り（低いほど良い）

| Mixer | スコア | 順位 |
|-------|--------|------|
| moremur | 0.002492 | 🥇 |
| rrmxmx | 0.002494 | 🥈 |
| nasam | 0.002502 | 🥉 |
| SplitMix64 | 0.002517 | 4 |
| MurmurHash3 | 0.002541 | 5 |

### 2. SAC（Strict Avalanche Criterion）

各入力ビット変化が出力ビットに与える影響の均一性

| Mixer | Worst | Avg | 総合 |
|-------|-------|-----|------|
| nasam | **0.0111** | 0.00251 | 🥇 |
| SplitMix64 | 0.0115 | 0.00257 | 🥈 |
| MurmurHash3 | 0.0118 | 0.00258 | 4 |
| rrmxmx | 0.0121 | 0.00253 | 🥉 |
| moremur | 0.0128 | **0.00250** | 5 |

### 3. BIC（Bit Independence Criterion）

出力ビット間の相関（低いほど良い）

| Mixer | スコア | 順位 |
|-------|--------|------|
| nasam | 0.00199 | 🥇 |
| rrmxmx | 0.00200 | 🥈 |
| moremur | 0.00200 | 🥉 |
| SplitMix64 | 0.00208 | 4 |
| MurmurHash3 | 0.00215 | 5 |

### 総合評価

| Mixer | 速度 | 品質 | バランス |
|-------|------|------|----------|
| MurmurHash3 | ⭐⭐⭐ | ⭐⭐ | 速度重視なら |
| SplitMix64 | ⭐⭐⭐ | ⭐⭐ | 定番 |
| **moremur** | ⭐⭐⭐ | ⭐⭐⭐ | **ベストバランス** |
| rrmxmx | ⭐⭐ | ⭐⭐⭐ | 中間 |
| nasam | ⭐⭐ | ⭐⭐⭐ | 品質最優先なら |

**結論**: moremurは速度がMurmurHash3並みで、品質は若干上。nasamは品質トップだが約24%遅い。ライブラリ用途にはmoremurがベストバランス。

---

## 追加検討アルゴリズム

### rrmxmx

Pelle Evensen による別のミキシング関数。

```typescript
function rrmxmx(x: bigint): bigint {
  x = x ^ ror64(x, 49n) ^ ror64(x, 24n);
  x = x * 0x9FB21C651E98DF25n & MASK64;
  x = x ^ (x >> 28n);
  x = x * 0x9FB21C651E98DF25n & MASK64;
  return x ^ (x >> 28n);
}
```

特徴: 回転を多用、moremurより若干遅いが品質は同等

### nasam

Pelle Evensen による高品質ミキシング関数。

```typescript
function nasam(x: bigint): bigint {
  x = x ^ ror64(x, 25n) ^ ror64(x, 47n);
  x = x * 0x9E6C63D0676A9A99n & MASK64;
  x = x ^ (x >> 23n) ^ (x >> 51n);
  x = x * 0x9E6D62D06F6A9A9Bn & MASK64;
  x = x ^ (x >> 23n) ^ (x >> 51n);
  return x;
}
```

特徴: 品質最優先、約24%低速

---

## 最終比較表

| アルゴリズム | 種別 | 速度 (ns/op) | 設計者 | 信頼性 | 推奨用途 |
|-------------|------|-------------|--------|--------|---------|
| moremur | Mixing | ~3.8 | Pelle Evensen | ✅ | **デフォルト** |
| SplitMix64 | Mixing | ~3.8 | Steele/Lea | ✅ Java標準 | 代替Mixing |
| XTEA | 暗号 | ~67 | Cambridge | ✅ 27年の実績 | **暗号化デフォルト** |
| Speck64 | 暗号 | ~7 | NSA | ⚠️ 論争あり | 高速オプション |
| Ascon | AEAD | N/A | Graz大学 | ✅ NIST標準 | **64bit不適** |

---

## 参考文献

### PCG
- Melissa E. O'Neill. "PCG: A Family of Simple Fast Space-Efficient Statistically Good Algorithms for Random Number Generation" (2014)
- https://www.pcg-random.org

### moremur
- Pelle Evensen. "Stronger, better, morer, moremur; a better mixer" (2020)
- https://mostlymangling.blogspot.com/2019/12/stronger-better-morer-moremur-better.html

### SplitMix64
- Guy L. Steele Jr., Doug Lea, Christine H. Flood. "Fast Splittable Pseudorandom Number Generators" (2014)
- http://dx.doi.org/10.1145/2714064.2660195
- Java 8 SplittableRandom: https://docs.oracle.com/javase/8/docs/api/java/util/SplittableRandom.html

### XTEA
- David J. Wheeler, Roger M. Needham. "Tea extensions" (1997)
- http://www.cix.co.uk/~klockstone/xtea.pdf

### wyhash
- Wang Yi. "wyhash and wyrand" (2019)
- https://github.com/wangyi-fudan/wyhash

### rrmxmx / nasam
- Pelle Evensen. "Better, stronger, mixer, and target the final avalanche" (2020)
- https://mostlymangling.blogspot.com/2020/01/nasam-not-another-strange-acronym-mixer.html

### Speck64
- Ray Beaulieu et al. "The SIMON and SPECK Families of Lightweight Block Ciphers" (2013)
- https://eprint.iacr.org/2013/404.pdf
- ISO/IEC 29167-22:2018（RFID標準としての採用）

### Ascon
- Christoph Dobraunig et al. "Ascon v1.2" (2021)
- https://ascon.iaik.tugraz.at/
- NIST Lightweight Cryptography Standardization (2023): https://csrc.nist.gov/projects/lightweight-cryptography

### 統計テスト
- PractRand: http://pracrand.sourceforge.net/
- TestU01 BigCrush: http://simul.iro.umontreal.ca/testu01/tu01.html
